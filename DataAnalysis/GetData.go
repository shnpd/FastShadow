...
func main() {
	...
	for round := 1; round <= 20; round++ {
		covertMsg := Crypto.GenerateRandomContent(16100)
		...
		skList1, err := msk1.DeprivCntKeys(client, 0, 501, netType)
		if err != nil {
			log.Fatal(err)
		}
		...
		skList2, err := msk2.DeprivCntKeys(client, 0, 501, netType)
		if err != nil {
			log.Fatal(err)
		}
		...
		covertTxid, err = SendCovertTransaction(client, addressList1, addressList2, 500, encryptMsg)
		if err != nil {
			fmt.Println(err)
			return
		}
		...
		for _, v := range covertTxid {
			...
			sig := Transaction.GetSigFromTx(client, v)
			signatures = append(signatures, sig)
		}
		fileoperator.SaveSignature(signatures, "DataAnalysis/DataSet/normalSig.xlsx")
