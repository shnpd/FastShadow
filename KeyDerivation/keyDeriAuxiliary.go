...
// FromWIF 将 WIF 格式转换为私钥
func FromWIF(wif string) ([]byte, string, error) {
	// 解码 Base58
...
func GetAddressByPrivKey(key *PrivateKey) (string, error) {
	prikWIF, err := ToWIF(key.Key)
	if err != nil {
		return "", err
	}
...
func GetAddressByPubKey(key *PublicKey) (string, error) {
	var param chaincfg.Params
	switch netType {
	case "simnet":
		param = chaincfg.SimNetParams
...
// GenerateEntireParentKey 基于密钥种子生成主密钥
func GenerateEntireParentKey(fatherKey *PublicKey, priKey []byte, id uint32) *PrivateKey {
	childIndexBytes := serializeUint32(id)
	data := fatherKey.Key
	data = append(data, childIndexBytes...)
	hmac := hmac.New(sha512.New, fatherKey.Chaincode)
	hmac.Write(data)
	I := hmac.Sum(nil)
	Chaincode := I[32:]
	key := &PrivateKey{
		Version:      testPrivateWalletVersion,
		FatherFinger: hash160(fatherKey.Key)[:4],
